...
class TransportException(Exception):
    def __init__(self, status, content, url, body):
        self.status = status
    ...
    def __str__(self):
        return repr(self)
    def __repr__(self):
        return "TransportException(%s): %s at %s sending %s" % (self.status, self.content, self.url, self.body)
...
class HttpTransport(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        self._attribute_stack = []
        return self._handle_response(response, data, url, body)
    ...
    def _handle_response(self, response, data, url, body):
        if response.status >= 400:
            raise TransportException(response, data, url, body)
        if not data:
          data = '{}'
